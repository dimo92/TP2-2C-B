...
const ACCUM_TYPE_BY_ROW = 'R';
const ACCUM_TYPE_BY_COL = 'C';

// Complete the organizingContainers function below.
function organizingContainers(containers) {
    // n x n | matrix => M[container][type]
    // # of types = # of containers = containers.length
    let accumRow = 0;
    let accumCol = 0;
    const rowsArray = [];
    const colsArray = [];
    ...
    try {
        accumRow = proccessMatrix(containerSize, containerSize, rowsArray, containers, 'R');
        accumCol = proccessMatrix(containerSize, containers.length, colsArray, containers, 'C');
        rowsArray.sort(function (a, b) { return b - a });
        colsArray.sort(function (a, b) { return b - a });

        let i = 0;
        let value = POSSIBLE;
        while (i < containerSize && value == POSSIBLE) {
            if (rowsArray[i] != colsArray[i]) {
                value = IMPOSSIBLE;
            }
            i++;
        }
...
function proccessMatrix(firstIndexTop, secondIndexTop, pushArray, inputArray, accumType) {
    let accum;

    if (accumType == ACCUM_TYPE_BY_ROW) {
        for (let r = 0; r < firstIndexTop; r++) {
            accum = 0;
            for (let c = 0; c < secondIndexTop; c++) {
                accum += inputArray[r][c];
            }
            pushArray.push(accum);
        }
    } else if (accumType == ACCUM_TYPE_BY_COL) {
        for (let c = 0; c < secondIndexTop; c++) {
            accum = 0;
            for (let r = 0; r < firstIndexTop; r++) {
                accum += inputArray[r][c];
